...
class MessageOrchestrator:
        ...
        async def _on_stream(update_obj: StreamUpdate) -> None:
            ...
            if not draft_streamer and verbose_level >= 1:
                ...
                if (now - last_edit_time[0]) >= 2.0 and tool_log:
                    ...
                    # markdown 표·코드블록 변환 + HTML 모드 — proportional font에서
                    # 표·코드 정렬 깨짐 방지 (OpenClaw 시절 해결책 복원)
                    from .utils.html_format import markdown_to_telegram_html
                    try:
                        await progress_msg.edit_text(
                            markdown_to_telegram_html(new_text),
    ...
    async def agentic_text(
        ...
        try:
            ...
            # Append cost/duration footer to last message
            if formatted_messages and claude_response:
                cost = claude_response.cost or 0.0
                duration_s = (claude_response.duration_ms or 0) / 1000
                turns = claude_response.num_turns or 0
                if cost > 0 or duration_s > 0:
                    footer = f"\n\n━━━\n💰 ${cost:.4f}  ⏱️ {duration_s:.1f}s  🔄 {turns} turns"
                    formatted_messages[-1].text += footer
