# COMPARISON — 참고 prototype vs Captain Hook 방향

## 두 시스템의 환경 차이

| 항목 | 참고 prototype (외부 레퍼런스) | Captain Hook (우리) |
|---|---|---|
...
**수단 갈림**:
- 참고 prototype: Claude Code CLI는 외부 hook만이 강제 발화 수단 → Stop Hook
- Captain Hook: 봇이 SDK wrapper → 봇 코드가 곧 외부 강제 레이어, Hook 불필요 (봇 안에서)

## 기능 매핑

| 참고 prototype 기능 | Captain Hook 대응 |
|---|---|
...
## 참고 prototype의 한계 — 우리 핵심 문제 미해결

**박제된 문제**: 백그라운드 작업이 turn 종료 *후* 완료 → 알림 누락

참고 prototype Stop Hook은 turn 종료 시점에만 발화. 백그라운드 작업 완료 시점은 그 *후*. Hook이 발화할 리 없음 → **참고 prototype 단독으론 못 푼다**.

Captain Hook 1층(stream-json 파서)이 이걸 해결: 백그라운드 작업 *시작* 시점에 cct-notifier 자동 등록 → 완료 시점에 cct-notifier가 polling 알림.

## 참고 prototype에서 차용할 부분
...
참고 prototype = **참고 자료**. 본체 운영 X. 핵심 패턴(Stop Hook, reply 매칭)만 우리 봇에 흡수. 진짜 알림 누락 문제는 봇 stream-json 파서가 푼다.

## 기존 방식이 풀지 않은 것 — captain-hook이 상위호환인 이유

| 누락 패턴 | 기존 Stop Hook | captain-hook |
|---|---|---|
...
기존 방식의 "100%"는 응답 누락 100%일 뿐. 백그라운드 누락은 미해결.

## 기존 Bridge daemon = captain-hook 어디로 흡수되는가
